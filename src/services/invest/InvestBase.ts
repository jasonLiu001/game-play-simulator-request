...
export class InvestBase {
    ...
    private async checkInvestTime(): BlueBirdPromise<any> {
        ...
        if (CONFIG_CONST.isRealInvest && TimeService.isReachInvestEndTime()) {
            ...
            //发送 购买结束提醒
            let sendEmailResult: any = await NotificationSender.send("购买完成，当前账号余额:" + investInfoList[0].currentAccountBalance, timeReachMessage, NotificationType.PUSH_AND_EMAIL);
    ...
    private async checkMaxWinMoney(): BlueBirdPromise<any> {
        ...
        if (currentAccountBalance >= CONFIG_CONST.maxAccountBalance) {
            if (CONFIG_CONST.isRealInvest) {//真实投注需要判断盈利金额设置
                ...
                //发送盈利提醒
                return await NotificationSender.send("达到目标金额:" + CONFIG_CONST.maxAccountBalance, winMessage, NotificationType.PUSH_AND_EMAIL);
            ...
            if (CONFIG_CONST.isRealInvest) {//真实投注需要判断亏损金额设置
                ...
                //发送亏损提醒
                return await NotificationSender.send("达到最低限额:" + CONFIG_CONST.minAccountBalance, loseMessage, NotificationType.PUSH_AND_EMAIL)
            }
        }
        return BlueBirdPromise.resolve(true);
    }
    ...
    private sendContinueInvestWarnEmail(): BlueBirdPromise<boolean> {
        ...
        return LotteryDbService.getInvestInfo(Config.globalVariable.last_Period, CONFIG_CONST.currentSelectedInvestPlanType)
            .then((lastInvestInfo: InvestInfo) => {
                ...
                //发送邮件提醒
                return NotificationSender.send("余额:" + lastInvestInfo.currentAccountBalance + "连续购买前", warnMessage, NotificationType.PUSH_AND_EMAIL);
            }).then(() => {
                return BlueBirdPromise.resolve(true);
            });
    }
