...
export class InvestBase {
    ...
    private async checkInvestTime(): BlueBirdPromise<any> {
        //检查在此时间内是否允许投注
        if (Tencent75TimeService.isInStopInvestTime()) {//不可投注的时间段时
            //更新开奖时间
        ...
        if (CONFIG_CONST.isRealInvest && Tencent75TimeService.isReachInvestEndTime()) {
    ...
    private sendContinueInvestWarnEmail(): BlueBirdPromise<boolean> {
        ...
        return InvestTableService.getInvestInfoByTableName(EnumDbTableName.INVEST, Config.globalVariable.last_Period, CONFIG_CONST.currentSelectedInvestPlanType)
            .then((lastInvestInfo: InvestInfo) => {
                ...
                let currentPeriodString: string = Tencent75TimeService.getCurrentPeriodNumber(new Date());
                //提醒邮件
    ...
    doCheck(): BlueBirdPromise<boolean> {
        ...
        return this.checkInvestTime()
            .then(() => {
                ...
                if (Tencent75TimeService.isInStopInvestTime() || Tencent75TimeService.isReachInvestEndTime()) return BlueBirdPromise.resolve();
